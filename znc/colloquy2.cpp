...
public:
	...
	virtual bool OnLoad(const CString& sArgs, CString& sErrorMsg) {
		...
		for(MCString::iterator it = BeginNV(); it != EndNV(); it++)
		{
			...
			} else if (it->first == "u:forcenotifychannel") {
				m_sForceNotifyChannel = it->second;
			}
		}
	...
	virtual void OnModCommand(const CString& sCommand) {
		if (sCommand.Equals("HELP")) {
			...
			PutModule("Command: SET forcenotifychannel <channel>");
			PutModule("  If set, will always notify for a channel on activity, even if doesn't match highlight keywords.");
			...
			} else if ( sKey == "forcenotifychannel" ) {
				m_sForceNotifyChannel=sCommand.Token(2);
				PutModule("Force highlight channel: '"+CString(m_sForceNotifyChannel)+"'");
			} else {
			...
			SetNV("u.forcenotifychannel", m_sForceNotifyChannel, false);
			...
			Table.AddRow();
			Table.SetCell("Option", "Force notify channel");
			Table.SetCell("Value", m_sForceNotifyChannel);
	...
	bool Push(const CString& sNick, const CString& sMessage, const CString& sChannel, bool bHilite, int iBadge) {
		if (iBadge != 0 && !m_bAttachedPush && m_pUser->IsUserAttached()) {
			if (m_debug) {
				PutModule("debug: won't push, user is attached!");
			}
			return false;
		}
		...
		if (iBadge != 0) {
			...
			if (pUser && m_bAwayOnlyPush && !pUser->IsIRCAway()) {
				if (m_debug) {
					PutModule("debug: won't push, user is not away!");
				}
				return false;
			}
		}
		...
		if ( m_bIgnoreNetworkServices ) {
			if ( sNick.Equals("NickServ") or sNick.Equals("ChanServ") or sNick.Equals("MemoServ") ) {
				if (m_debug) {
					PutModule("debug: won't push, from network services!");
				}
				return false;
			}
		}
		...
		if (bNightHours) {
			if (m_debug) PutModule("debug: won't push, night hours!");
			return false;
		}
		...
		if (bIsNotIdle) {
			if (m_debug) PutModule("debug: won't push, not idle!");
			return false;
		}
		...
		for (map<CString, CDevice*>::iterator it = m_mspDevices.begin(); it != m_mspDevices.end(); it++) {
			...
			if (bFound && iBadge != 0) {
				if (m_debug) PutModule("debug: won't push, mobile device connected!");
				return false;
			}
			...
			if (bHilite) {
				...
				if (!bMatches && !sChannel.Equals(m_sForceNotifyChannel)) {
					if (m_debug) PutModule("debug: won't push, no highlite match and not forced channel!");
					return false;
			...
			if (!pDevice->Push(sNick, sPushMessage, sChannel, bHilite, iBadge)) {
				if (m_debug) PutModule("debug: push failed!");
				bRet = false;
			}
			else
			{
				if(m_debug) PutModule("debug: push sent!");
			}
		}
